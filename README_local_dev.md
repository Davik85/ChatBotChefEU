...
2. Запустите приложение (`./gradlew run` или Run Configuration в IDE). При значении `TELEGRAM_TRANSPORT=LONG_POLLING` конфиг автоматически загрузится из `.env`, поэтому отдельный `source` не требуется. Если какие-то значения нужно переопределить, их можно задать через переменные окружения.
